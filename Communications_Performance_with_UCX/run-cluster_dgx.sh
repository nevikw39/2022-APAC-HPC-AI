...
if [ "$SERVER_PROC" = true ]; then
    LOG_FILE=${JOB_DIR}/server-${PBS_JOBID}.log
    OUT_FILE=${JOB_DIR}/server-${PBS_JOBID}.out
else
    LOG_FILE=${JOB_DIR}/worker-${PBS_JOBID}-${HOSTNAME}.log
    OUT_FILE=${JOB_DIR}/worker-${PBS_JOBID}-${HOSTNAME}.out
fi
